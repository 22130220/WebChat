...
const MEDIA_STYLE =
  "max-w-[300px] w-full rounded-xl object-cover cursor-pointer shadow-sm";
...
const MessageContent = ({
    ...
    return (
      <div className="flex flex-col">
        <p className="text-sm whitespace-pre-wrap break-words">
          {safeDecodeURIComponent(msg.content)}
        </p>
        {detectUrl && (
  ...
  const renderContent = () => {
    switch (msg.type) {
      ...
      default:
        return (
          <p className="text-sm">{safeDecodeURIComponent(msg.content || "")}</p>
        );
    }
  };
